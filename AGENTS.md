...
- **Bot**: 55 тестов (notifications, referrals, login token, bind code, login code, context-aware menu).
- Все тесты проходят: `go test ./...`, `npm test -- --run`, `python3 -m pytest test_bot.py`.

### 4.4 UX/Conversion улучшения бота (2026-09-09)
- **Контекстное меню**: `get_user_menu_keyboard(telegram_id)` динамически упорядочивает кнопки:
  - для истёкшего/отсутствующего ключа — «🛒 Купить тариф» и «🔑 Получить пробный ключ» первыми;
  - для активной подписки — «📊 Моя подписка (осталось N дн.)» и «🛒 Купить тариф» первыми.
- **Счётчик дней в меню**: текст кнопки «Моя подписка» теперь включает остаток дней/статус.
- **Retargeting через 3 дня**: добавлен `send_retargeting_notifications()` — через 72–96 часов
  после истечения отправляет «🥹 Мы скучаем по вам! +2 дня при покупке».
- **Промо-ощущение рефералов**: уведомление `referral_reward` переименовано в
  «🎁 Промокод активирован! +N дней» для усиления ценности бонуса.
- **Прямая ссылка на тарифы**: добавлен `PRICING_WEB_APP_URL`, кнопка «🛒 Купить тариф»
  открывает `/pricing` напрямую из любого меню.
- Убраны агрессивные формулировки из уведомлений об истечении; оставлены конкретные CTA
  и подсказки по WiFi/мобильному хотспоту.
